...
import com.wyb.aicodemotherme.mapper.UserMapper;
...
import static com.wyb.aicodemotherme.constant.UserConstant.USER_LOGIN_STATE;
...
public class UserServiceImpl extends ServiceImpl<UserMapper, User> implements UserService{
    ...
    public long userRegister(String userAccount, String userPassword, String checkPassword) {
        ...
        if(userAccount.length() < 4){
            throw new BusinessException(ErrorCode.PARAMS_ERROR, "用户账户不能小于4位");
        }
        ...
        //2.检查是否重复
        long count = count(new QueryWrapper<User>().eq("userAccount", userAccount));
        ...
        boolean saveRueslt = this.save(user);
        if(!saveRueslt){
    ...
    public List<UserVO> getUserVOList(List<User> userList) {
        ...
        //map 的作用是：对流里的每一个元素做“映射/转换”，把它变成另一种东西
        //this::getUserVO 表示：对每个 User 调用当前类的 getUserVO(User u) 方法
        //等价于.map(user -> this.getUserVO(user))
    ...
    public QueryWrapper<User> getQueryWrapper(UserQueryRequest userQueryRequest) {
        ...
        //如果 userAccount 有值，则拼 AND userAccount LIKE '%xxx%'（MP 默认会自动加 %）
